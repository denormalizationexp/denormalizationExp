...
int		partial (int, int);
int		refresh_supported_table(int tbl);
int		refresh_seed_table(int tbl);
DSS_HUGE	refresh_row_start(int tbl, DSS_HUGE rowcnt, long upd_num);
...
usage (void)
{
	...
	fprintf (stderr, "-U <n> -- generate <n> update sets; default target is orders/lineitem,\n");
	fprintf (stderr, "          use -T to generate refresh data for a specific table\n");
	...
	fprintf (stderr, "\nTo generate refresh data for part only, use:\n");
	fprintf (stderr, "\tdbgen -v -U 1 -T P -s 1\n");
}
...
int
refresh_supported_table(int tbl)
{
	switch (tbl)
	{
	case PART:
...
DSS_HUGE
refresh_row_start(int tbl, DSS_HUGE rowcnt, long upd_num)
{
	int base_tbl;

	if ((tbl == ORDER) || (tbl == LINE) || (tbl == ORDER_LINE))
		return (upd_num * rowcnt + 1);

	base_tbl = refresh_seed_table(tbl);
	return ((tdefs[base_tbl].base * scale) + (upd_num * rowcnt) + 1);
}
...
void validate_options(void)
{
	...
	// DBGenOptions, 3.10
	if ((updates != 0) && bTableSet)
	{
		int i;
		int found = 0;

		for (i = PART; i <= REGION; i++)
			if ((table & (1 << i)) && refresh_supported_table(i))
				found = 1;

		if (!found)
		{
			fprintf(stderr, "ERROR: -T with -U only supports customer, supplier, part, partsupp, orders, lineitem, part/partsupp, or orders/lineitem\n");
			exit(-1);
		}
	}
...
main (int ac, char **av)
{
	...
	*/
	if (updates)
		...
		for (refresh_tbl = PART; refresh_tbl <= REGION; refresh_tbl++)
			{
			if (!(table & (1 << refresh_tbl)))
				continue;
			if (!refresh_supported_table(refresh_tbl))
				continue;

			seed_tbl = refresh_seed_table(refresh_tbl);

			/* 
			 * set RNG to start generating rows beyond SF=scale
			 */
			set_state (seed_tbl, scale, 100, 101, &i); 
			rowcnt = (int)(tdefs[refresh_tbl].base / 10000 * scale * UPD_PCT);
			if (step > 0)
				{
				/* 
				...
				for (i=1; i < step; i++)
				{
					tdefs[seed_tbl].gen_seed(0, rowcnt);
					if ((seed_tbl == LINE) || (refresh_tbl == ORDER_LINE))
						sd_line(0, rowcnt);
					else if (tdefs[seed_tbl].child != NONE)
						tdefs[tdefs[seed_tbl].child].gen_seed(0, rowcnt);
				}
				upd_num = step - 1;
				}
			else
			...
			while (upd_num < updates)
				...
				if (verbose > 0)
					...
					upd_num + 1, tdefs[refresh_tbl].comment);
				insert_orders_segment=0;
				insert_lineitem_segment=0;
				delete_segment=0;
				minrow = refresh_row_start(refresh_tbl, rowcnt, upd_num);
				gen_tbl (refresh_tbl, minrow, rowcnt, upd_num + 1);
				if (verbose > 0)
					fprintf (stderr, "done.\n");
				switch (refresh_tbl)
				{
				case ORDER_LINE:
					pr_drange (ORDER, minrow, rowcnt, upd_num + 1);
					pr_drange (LINE, minrow, rowcnt, upd_num + 1);
					break;
				case PART_PSUPP:
					pr_drange (PART, minrow, rowcnt, upd_num + 1);
					pr_drange (PSUPP, minrow, rowcnt, upd_num + 1);
					break;
				default:
					pr_drange (refresh_tbl, minrow, rowcnt, upd_num + 1);
					break;
				}
				upd_num++;
				}
