		...
		if (update > 0) /* updates */
			if ( insert_segments )
				...
				if ( delete_segments )
					{
					++delete_segment;
					sprintf(upath, "%s%cdelete.%s.u%d.%d",
						env_config(PATH_TAG, PATH_DFLT), PATH_SEP, tdefs[table].name,
						-update%10000, delete_segment);
					}
				else
					{
					sprintf(upath, "%s%cdelete.%s.%d",
					env_config(PATH_TAG, PATH_DFLT), PATH_SEP, tdefs[table].name, -update);
					}
...
static int last_mode = 0;
   ...
   if (fp == NULL || mode != last_mode)
        {
        if (fp)
            fclose(fp);
        fp = print_prep(CUST, mode);
        last_mode = mode;
        }
...
static int last_mode = 0;
    ...
    if (p_fp == NULL || mode != last_mode)
        {
        if (p_fp)
            fclose(p_fp);
        p_fp = print_prep(PART, mode);
        last_mode = mode;
        }
...
pr_drange(int tbl, DSS_HUGE min, DSS_HUGE cnt, long num)
{
    ...
    if ((tbl == ORDER) || (tbl == LINE) || (tbl == ORDER_LINE))
        start = MK_SPARSE(min, 1 + ((num - 1) / (10000 / UPD_PCT)));
    else
        start = min;
    last = start - 1;
	...
	{
		if ((tbl == ORDER) || (tbl == LINE) || (tbl == ORDER_LINE))
			new = MK_SPARSE(child, 1 + ((num - 1) / (10000 / UPD_PCT)));
		else
			new = child;
		if (delete_segments)
		{
