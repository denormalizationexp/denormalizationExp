...
from job_utils import build_jobs_from_templates, load_template_module
...
# Query-driven is the superset used for query-driven experiments: templates
# with query-specific edges plus a copied schema-driven template set.
TEMPLATES_DIR = Path(__file__).resolve().parents[1] / "templates" / "query_driven"
RESULTS_DIR = Path(__file__).resolve().parents[1] / "results" / "query_driven"
...
def main():
    ...
    for job in jobs:
        ...
        mv_query = TEMPLATE_SQL.format(source=mv_src, **params_list[0])
        validate_equivalence(f"MV Q{qid} ({tag})", mv_query, baseline_result)

        # Benchmark
        key = f"Q{qid}_{tag}"
        results[key] = {}
        results[key]["baseline"] = benchmark(f"Baseline Q{qid} ({tag})", None, BASELINE_SQL, params_list)
        results[key]["materialized_view"] = benchmark(f"MV Q{qid} ({tag})", mv_src, TEMPLATE_SQL, params_list)

        # save per job
        out_path = RESULTS_DIR / f"result_{mv_src}.json"
