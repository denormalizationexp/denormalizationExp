...
from job_utils import SYM2TABLE, load_template_module
...
# Schema-driven multi templates contain only combinations whose component
# templates all come from schema_driven.
TEMPLATES_DIR = Path(__file__).resolve().parents[1] / "templates_multiple" / "schema_driven"
RESULTS_DIR = Path(__file__).resolve().parents[1] / "results_multiple" / "schema_driven"
...
def build_mv_source(qid: int, component_tag: str) -> str:
    try:
    ...
    return f"q{qid}_mv_" + "_".join(table_parts)


def build_jobs_from_multi_templates(templates_dir: Path) -> list[MultiJob]:
    ...
    for path in sorted(templates_dir.glob("template_*.py")):
        ...
        source_map = {
            f"source{idx}": build_mv_source(qid, component_tag)
...
def main() -> None:
    ...
    for job in jobs:
        ...
        mv_query = format_query(template_sql, job.source_map, params_list[0])
        validate_equivalence(cur, f"Multi-MV Q{qid} ({tag})", mv_query, baseline_result)

        key = f"Q{qid}_{tag}"
        results[key] = {}
        results[key]["baseline"] = benchmark(cur, f"Baseline Q{qid} ({tag})", None, baseline_sql, params_list)
        results[key]["materialized_view"] = benchmark(
            cur,
            f"Multi-MV Q{qid} ({tag})",
            job.source_map,
            template_sql,
            params_list,
        )
    ...
    all_path = RESULTS_DIR / "all_results_multi.json"
