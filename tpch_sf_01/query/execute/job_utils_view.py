...
def build_jobs_from_templates(
    ...
    view_override: Optional[Dict[str, str]] = None,
) -> List[Job]:
    """
    ...
    view_src default rule:
        q{qid}_view_<table names joined by underscore>
        e.g., template_2_PS_S.py -> q2_view_partsupp_supplier
    """
    view_override = view_override or {}
    ...
    for p in sorted(templates_dir.rglob("template_*.py")):
        ...
        view_src = f"q{qid}_view_" + "_".join(table_parts)
        view_src = view_override.get(p.name, view_src)

        template_file = p.relative_to(templates_dir).as_posix()
        jobs.append(Job(qid=qid, tag=tag, template_file=template_file, view_src=view_src))
